...
public:
    ...
    cv::Mat processImageAndEstimatePose(const std::string& imagePath, const std::vector<float>& bbox,
        ...
        // ransacVoting::ransacVotingV4(mask, vertex, outputPoints, 16, 0.999, 0.99, 20, 5, 30000);
...
int main() {
    ...
    */
    for(auto imagePath : imagePaths){
        try {
            // std::string imagePathTest = "/home/nvidia/work/pvnet_deploy/data/new_nocrop_0116/rgb/247.jpg";
            std::cout << "Image:" << std::filesystem::path(imagePath).stem().string() << std::endl;
            int image_index = std::stoi(std::filesystem::path(imagePath).stem().string());  // 示例转换为int
            std::vector<float> bbox = getBBoxFromJson(image_index, train_json_file);
            ...
            /*
                save result RT Matrix to .npy file
            */
            // std::string outputFilePath = outputFolder + "/" + std::filesystem::path(imagePath).stem().string() + ".npy";
            // savePoseMatrixToNpy(poseMatrix, outputFilePath);
            // std::cout << "Saved pose matrix to " << outputFilePath << std::endl;
            // break;
